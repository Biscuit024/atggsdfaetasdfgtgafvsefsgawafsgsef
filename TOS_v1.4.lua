...
x6='705298434;2063742506;2755072;274451;227;671222294'
x7='50463744;33554432;33554432;1912668160'
...
gg.searchNumber(x6, gg.TYPE_DWORD)
	if gg.getResultCount()==0 then
		print('※怪物無技能 [尋找數據失敗]※   ※Enemy without skill [Fail to modified]※')
	else
		r = gg.getResults(1,2)
		address1 = (r[1].address -4)
		gg.clearResults()
		gg.searchAddress(string.format("%X", address1), 0xFFFFFFFF,gg.TYPE_BYTE)
		r = gg.getResults(1)
		gg.editAll('22', gg.TYPE_BYTE)
		gg.clearResults()
		print("※怪物無技能 [修改成功]※   ※Enemy without skill [Successfully modified]※")
		gg.alert('為減少使用者在使用敵方無技能修改下被封帳，請在[首次進入關卡後]而且在[轉珠前]，重啟腳本點選[數據改回功能]再按[③敵方無技能數據改回]。')
	end
gg.clearResults()
end
...
function mercenary ()
gg.setVisible(false) 
gg.searchNumber(x7, gg.TYPE_DWORD)
	if gg.getResultCount()==0 then
	print('※全戰友 [尋找數據失敗]※   ※All mercenary [Fail to modified]※')
	else
		r = gg.getResults(1,6)
		address1 = (r[1].address +4)
		address2 = (r[1].address +12)
		address3 = (r[1].address +20)
		gg.clearResults()
		gg.searchAddress(string.format("%X", address1), 0xFFFFFFFF,gg.TYPE_QWORD)
		r = gg.getResults(1)
		gg.editAll('17189432701', gg.TYPE_QWORD)
		gg.clearResults()
		gg.searchAddress(string.format("%X", address2), 0xFFFFFFFF,gg.TYPE_QWORD)
		r = gg.getResults(1)
		gg.editAll('0', gg.TYPE_QWORD)
		gg.clearResults()
		gg.searchAddress(string.format("%X", address3), 0xFFFFFFFF,gg.TYPE_QWORD)
		r = gg.getResults(1)
		gg.editAll('-7930985170256003072', gg.TYPE_QWORD)
		gg.clearResults()
		print("※全戰友 [修改成功]※   ※All mercenary [Successfully modified]※")
	end
gg.clearResults()
end
...
function emnoskillreset ()
g.setVisible(false) 
gg.searchNumber(x6, gg.TYPE_DWORD)
	if gg.getResultCount()==0 then
		print("※怪物無技能 [數據改回失敗]※   ※Enemy without skill [Fail to restore]※")
	else
		r = gg.getResults(1,2)
		address1 = (r[1].address -4)
		gg.clearResults()
		gg.searchAddress(string.format("%X", address1), 0xFFFFFFFF,gg.TYPE_BYTE)
		r = gg.getResults(1)
		gg.editAll('3', gg.TYPE_BYTE)
		gg.clearResults()
		print("※怪物無技能 [數據改回成功]※   ※Enemy without skill [Successfully restored]※")
	end
gg.clearResults()
end
...
function Extra ()
if leng == 1 then
	local funcTable= gg.multiChoice({'1.MISS','2.全體攻擊','3.CD 1','4.怪物無技能','5.戰友','6.數據改回功能'}, {}, '選擇額外功能')
		if not funcTable then
			gg.toast("已取消使用額外功能")
			local leave= gg.choice({'返回主要功能','離開腳本'})
			gg.setRanges(bit32.bxor(gg.REGION_C_ALLOC,gg.REGION_ANONYMOUS))
...
if leng == 2 then
	local funcTable= gg.multiChoice({'1.MISS','2.Allatk','3.CD 1','4.Enemy without skill','5.All mercenary','6.Data restore'}, {}, 'Extra function choices')
		if not funcTable then
...
function tosreset ()
if leng == 1 then
		local funcTable= gg.multiChoice({'①Combo數據改回','②CD1數據改回','③敵方無技能數據改回'}, {}, '選擇數據改回功能')
		if not funcTable then
			gg.toast("取消使用數據改回功能")
			local leave= gg.choice({'返回主要功能','離開腳本'})
			gg.setRanges(bit32.bxor(gg.REGION_C_ALLOC,gg.REGION_ANONYMOUS))
...
if leng == 2 then
		local funcTable= gg.multiChoice({'①Combo% restore','②CD1 restore','③Enemy without skill restore'}, {}, 'Data restore choices')
		if not funcTable then
